...
#ifdef _WIN32
/*!
  @brief Like fileProtocol(const std::string& path) but accepts a unicode path
      in an std::wstring.
...
#ifdef _WIN32
/*!
  @brief Like fileExists(const std::string& path) but accepts a unicode path
      in an std::wstring.
  @note This function is only available on Windows.
 */
EXIV2API bool fileExists(const std::wstring& wpath);
#endif
